...
CompareClustersByTrt <- function(obj, trt_var, rep_var=NULL, group_by=NULL, ci=TRUE, ci_alpha=0.05, seed=1) {
	...
	if (is.null(rep_var)) {
		grp_dat <- dplyr::group_by(meta, !!trt_var) %>%
			dplyr::mutate(N_tot=dplyr::n()) %>% dplyr::ungroup() %>%
			dplyr::group_by(!!trt_var, ident, N_tot)
	} else {
		rep_var <- rlang::enquo(rep_var)
		grp_dat <- dplyr::group_by(meta, !!trt_var, !!rep_var) %>%
			dplyr::mutate(N_tot=dplyr::n()) %>% dplyr::ungroup() %>%
			dplyr::group_by(!!trt_var, !!rep_var, ident, N_tot)
	}
	grp_dat <- grp_dat %>% dplyr::summarize(N=dplyr::n()) %>% 
	...
	grp_stats <- dplyr::mutate(grp_dat, lower=qbeta(ci_alpha/2, N + 1/2, N_tot - N + 1/2)) %>%
  		dplyr::mutate(upper=qbeta(1-ci_alpha/2, N + 1/2, N_tot - N + 1/2)) %>%
 		dplyr::mutate(cells_per_thousand=frac*1000) %>%
  		dplyr::mutate(lower_per_thousand=lower*1000, upper_per_thousand=upper*1000)
  	# Get some summary stats
	cfrac <- grp_stats %>% dplyr::group_by(ident) %>%
